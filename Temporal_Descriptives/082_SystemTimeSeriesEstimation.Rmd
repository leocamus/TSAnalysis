...
Variables to test:
pn_SUM_EXP_no_t, pn_SUM_EXP_3_t, pn_SUM_EXP_tm, N_ZPs, kms_ofertados, TOTAL_trx.
...
Plotting some variables in time : line plot of total number of supplied kms.
```{r}
kms_plot <- ggplot(complete_db, aes(x=complete_db$DATE, y=complete_db$kms_ofertados)) + 
  ...
  ylab("supplied-kms") + 
  scale_x_date(date_breaks="2 months") + 
#  scale_y_continuous(breaks = seq(min(complete_db$kms_ofertados), max(complete_db$kms_ofertados), by = 1000), label=scales::comma) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) #+ 
#  coord_fixed(ratio = 1/5000)
...
```{r}
complete_db[which(complete_db$kms_ofertados>=1500000),c("DATE","kms_ofertados")]
```

Checking for inconcistency in kms-ofertados
```{r}
marzo_2015 <- read.table("/home/leonardo/Desktop/Evasion/01_analisis/03_datos/04_DTPM/06_LBS/0_kms_recorridos/2015/marzo_kms_recorridos.txt", header=TRUE, sep=";", stringsAsFactors = FALSE, dec=",")

marzo_2015$Fecha <- as.Date(marzo_2015$Fecha,"%d/%m/%Y")
marzo_2015$UN <- factor(marzo_2015$UN)
#marzo_2015$Expr1002 <- as.numeric(marzo_2015$Expr1002)

ggplot(marzo_2015, aes(x=marzo_2015$Fecha, y=marzo_2015$Expr1002)) + 
  geom_line(aes(col=marzo_2015$UN)) + 
  xlab("date (days)") + 
  ylab("supplied-kms") + 
