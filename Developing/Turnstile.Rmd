...
$T = c + t_{L} \cdot L + t_{s}\cdot S + \Big( t_{Y_{P}} + t_{Y_{P}}^{'} \cdot \tau_{3B} + \big(t_{Y_{P}}^{''} + t_{Y_{P}}^{'''} \cdot \tau_{peak}\big) \tau_{M}\Big) \cdot Y_{P} + t_{ZP} \cdot Y_{ZP} + N^{I} + N_{peak}^{I}$
...
$T = c + t_{L} \cdot L + t_{s}\cdot S + \Big(t_{Y} + t_{Y}^{'} \cdot \tau_{3B} + (t_{Y}^{''} + t_{Y}^{'''} \cdot \tau_{peak}) \tau_{M}\Big) \cdot Y + N^{I} + N_{peak}^{I}$
...
#Ingreso por el resto de las puertas
Y_resto <- database$I.P2 + database$I.P3 + database$I.P4

#Dataframe
data.all <- data.frame(database$TEXP, database$L.m., database$DET, Y_P1, Y_P1_3b, Y_P1_marip, Y_P1_marip_peak, Y_resto, database$NI, NI_peak)
names(data.all) <- c("texp","largo","detenciones","Y_P1","Y_P1_3B","Y_P1_Marip","Y_P1_Marip_peak", "Y_resto","NI","NI_peak")
